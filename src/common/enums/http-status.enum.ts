export enum HttpStatus {
  /**
   * The initial part of the request has been received by the server and that the client
   * should proceed with the request or ignore the response if the request has already
   * finished.
   */
  CONTINUE = 100,

  /**
   * The server understands the Upgrade header field request and indicates which protocol it
   * is switching to.
   */
  ...
  /**
   * The request was successfully fulfilled and resulted in one or possibly multiple new resources
   * being created.
   */
  CREATED = 201,
   ...
   * The request may or may not be completed when the processing eventually takes place.
   */
  ACCEPTED = 202,

  /**
   * The request was successful. However, the meta-information that has been received is different
   * from the one on the origin server and has instead been collected from a 3rd party or local
   * copy. When not used for backups or mirrors of another resource a 200 OK response is preferable.
   *
   * @since HTTP/1.1
   */
  ...
  /**
   * The server has successfully fulfilled the request, there is no available content for this
   * request. But the user agent might want to update its currently cached headers for this resource,
   * for the new one.
   */
  NO_CONTENT = 204,

  /**
   * The user should reset the document that sent this request because the server successfully
  ...
  /**
   * This is a response to a Range header sent from the client when requesting only a part of
   * the resource because the server is delivering only part of the resource (byte serving) due
   * to a range header sent by the client.
   * The range header is used by HTTP clients to enable resuming of interrupted downloads, or split
   * a download into multiple simultaneous streams.
   */
  PARTIAL_CONTENT = 206,

  /**
   * Conveys information about multiple resources, in situation when multiple status codes are
   * appropriate.
   * This is a response to a Range header sent from the client when requesting only a part of the resource.
   ...
   * (e.g., malformed request syntax, too large size, invalid request message framing, or deceptive request
   * routing).
   */
  BAD_REQUEST = 400,
   ...
   * not yet been provided. The response must include a WWW-Authenticate header field containing a challenge
   ...
   * for example, a GET request on a form that requires data to be presented via POST, or a PUT request on
  ...
  /**
   * The request is larger than the server is willing or able to process. Previously called "Request Entity
   * Too Large".
   */
   ...
   * For example, the client uploads an image as image/svg+xml, but the server requires that images use a
   * different format.
   */
   ...
   * and is not expected to be implemented by actual HTTP servers. The RFC specifies this code should be
   * returned by teapots requested to brew coffee. This HTTP status is used as an Easter egg in some websites,
